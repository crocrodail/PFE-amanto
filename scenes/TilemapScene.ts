...
class TilemapScene extends Phaser.Scene {
  // The unique id fpr the tilemap.
  // Assume the tilemap json is located at <tilemapKey>.json
  private tilemapKey: string;

  // The unique id for the tileset
  // Assumes the tileset image is located at <tilesetKey>.png
  ...
  public init(data: any): void {
    this.tilemapKey = "assets/map";
    this.tilesetKey = "assets/tiles";
    this.animatedTiles = [];
  }
  ...
  public create(data: any): void {
    ...
    const tileset = tilemap.addTilesetImage(tilesetName, this.tilesetKey);
...
class Player extends Phaser.GameObjects.Sprite {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    this.scene.anims.create({
      key: "idle",
      frames: this.scene.anims.generateFrameNames("assets/elf_f", {
        prefix: "idle_anim_f",
        end: 3
      }),
      frameRate: 8
